...
### Fixed
- **App name now shows as "Home Rec"** — Set `CFBundleDisplayName` (via `INFOPLIST_KEY_CFBundleDisplayName`) so the Finder/Desktop label, Dock tile, and macOS app menu read **"Home Rec"** (with a space) instead of "HomeRec", matching the window title and brand. (A pre-existing `PRODUCT_BUNDLE_DISPLAY_NAME` setting was a no-op — not a recognized build setting.) Bundle identifier, executable, and target are unchanged. (BL-070)

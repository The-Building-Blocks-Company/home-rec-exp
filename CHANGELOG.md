...
### Added
- **FLAC recording** — A third format, alongside WAV and M4A. FLAC is lossless like WAV — the audio is identical, nothing is thrown away — but the files are meaningfully smaller, typically around half the size for speech and music (and far smaller for simple content). Pick it from the format selector. One caveat worth knowing: unlike WAV and M4A, a FLAC recording interrupted by a crash or force-quit cannot currently be played back, because the format only writes the information a player needs at the moment you press stop. If that matters more than file size, stay on WAV. (BL-013)
